...
def coord_home(request):
    ...
    if(request.method=='POST'):
        try:
            ...
            return HttpResponseRedirect('/answers/'+str(choiceset.get(pref_no=ch).event))
        form=Preferenceform(request.POST)
        if form.is_valid():
            if choiceset.count()==0:
                choice1=Choice(user=request.user,pref_no=1,event=Event.objects.get(id=form['preference1'].value))
                choice2=Choice(user=request.user,pref_no=2,event=Event.objects.get(id=request.POST['preference2']))
                choice3=Choice(user=request.user,pref_no=3,event=Event.objects.get(id=request.POST['preference3']))
            else:
                choice1=choiceset.get(pref_no=1)
                choice1.event=Event.objects.get(id=request.POST['preference1'])
                choice2=choiceset.get(pref_no=2)
                choice2.event=Event.objects.get(id=request.POST['preference2'])
                choice3=choiceset.get(pref_no=3)
                choice3.event=Event.objects.get(id=request.POST['preference3'])
            if choice1.event_id ==1:
                if choice2.event_id>1 or choice3.event_id >1:
                    return HttpResponse("first pref can't be none when 2nd and 3rd pref are filled ")
            else:
                if choice2.event_id==1 and choice3.event_id>1:
                    return HttpResponse("2nd pref can't be none when 3rd pref is filled ")
            if choice1.event_id>1 and choice2.event_id>1 or choice3.event_id>1:
                if choice1.event_id==choice2.event_id or choice2.event_id==choice3.event_id or choice1.event_id==choice3.event_id:
                    return HttpResponse("No two choices can be same")
            choice1.save()
            choice2.save()
            choice3.save()     
            return render_to_response("coord_home.html",{'user':user,'PreferenceForm':form,'selected':True,'saved':True},context_instance= RequestContext(request))
        else:
            return render_to_response("coord_home.html",{'user':user,'PreferenceForm':form,'selected':False,},context_instance= RequestContext(request))
    if choiceset.count() == 0:
        form=Preferenceform()
    else:
    ...
    return render_to_response("coord_home.html",{'user':user,'PreferenceForm':form,'selected':choiceset.count()},context_instance= RequestContext(request)) 
